...
pub async fn handshake(server: &str, psk: &str) -> anyhow::Result<HandshakeResult> {
    ...
    let tls_config = crate::tls::chrome_tls_config(true)?;
    ...
    ws_write.send(WsMessage::Binary(auth_frame)).await?;
                    ...
                    match read_result {
                        Ok(n) if n > 0 => {
                            match encoder.encode_data(&buf[..n]) {
                                Ok(frame_data) => {
                                    stats_tx.tx_bytes.fetch_add(n as u64, Ordering::Relaxed);
                                    stats_tx.tx_packets.fetch_add(1, Ordering::Relaxed);
                                    if ws_write
            ...
            msg = ws_read.next() => {
                match msg {
                    Some(Ok(WsMessage::Binary(data))) => {
                        match decoder.decode_owned(data) {
                            Ok(Frame::Data(ip_packet)) => {
                                stats.rx_bytes.fetch_add(ip_packet.len() as u64, Ordering::Relaxed);
