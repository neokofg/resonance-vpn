...
pub async fn run(opts: DeployOpts) -> anyhow::Result<()> {
    ...
    if status.stdout.trim() != "active" {
        log::warn!("Service may not have started correctly. Check: journalctl -u {SERVICE_NAME}");
    }
...
async fn download_server_binary(session: &SshSession, arch: &str) -> anyhow::Result<()> {
    ...
    if existing.exit_code == 0 {
        log::info!(
            "Server binary already present at {INSTALL_DIR}/resonance-server, skipping download."
        );
        return Ok(());
    }
    ...
    session
        ...
        .await
        .context(
            "Failed to download server binary from GitHub Releases. \
