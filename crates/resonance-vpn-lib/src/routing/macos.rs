...
impl RoutingState {
    pub fn setup(
        ...
        // Route VPN server traffic through original gateway
        run_cmd("route", &["-n", "add", "-host", &server_ip, &original_gateway])?;

        // Split-route: 0.0.0.0/1 + 128.0.0.0/1 covers all traffic
        run_cmd("route", &["-n", "add", "-net", "0.0.0.0/1", "-interface", tun_name])?;
        run_cmd("route", &["-n", "add", "-net", "128.0.0.0/1", "-interface", tun_name])?;

        let mut original_dns_service = None;
        let mut original_dns_servers = None;

        if !dns.is_empty() {
            // Find primary network service and save current DNS
            if let Ok(service) = get_primary_network_service() {
                ...
                // networksetup is at a known path, call via run_cmd
    ...
    pub fn cleanup(&self) {
        ...
        if let Some(ref service) = self.original_dns_service {
            if let Some(ref servers) = self.original_dns {
                ...
                // Reset to DHCP-assigned DNS ("Empty" clears manual DNS)
...
fn get_primary_network_service() -> anyhow::Result<String> {
    let output = Command::new("networksetup")
        ...
        .output()?;
    let stdout = String::from_utf8_lossy(&output.stdout);
    // Skip the first line (header), find first non-disabled service
    let service = stdout
        .lines()
        .skip(1)
